...
export default function NewsletterPopup() {
  ...
  useEffect(() => {
    ...
    function handleKeyDown(e: KeyboardEvent) {
      ...
      // Only keep the last 4 characters
      if (typed.length > 4) {
        typed = typed.slice(-4);
      }
  ...
  function handleSubmit(e: FormEvent) {
    ...
    // Demo only: no email service is connected.
    setStatus("success");
  }
            ...
            <form onSubmit={handleSubmit} className="space-y-3">
              ...
              {status === "error" && (
                <p className="text-sm text-compass" role="alert">
                  That email doesn't look quite right — please check it and try
                  again.
                </p>
              )}
